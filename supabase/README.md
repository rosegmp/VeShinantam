...
Authentication uses email and password. If email confirmation is enabled, add the deployed site URL and local development URL to the Supabase Auth redirect allow list. The browser stores its refresh token locally, while Row Level Security restricts each snapshot to its authenticated owner.
