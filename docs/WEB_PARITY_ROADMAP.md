...
- Sync policy tests cover retry classification, duplicate acknowledgements, concurrent edit/delete conflicts, mutation rebasing, account-switch rejection, and token-refresh boundaries. A deterministic two-device gate runs independent IndexedDB stores through the deployed server contract for seeding, offline edits, remote-wins conflicts, tombstones, interrupted acknowledgements, idempotent replay, and late-ack replacement rebasing. Live authenticated Android/web and end-to-end token refresh complete this gate.
