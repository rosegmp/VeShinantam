...
Status: implemented feature roadmap; release hardening remains in progress
...
Implementation status (updated 2026-09-23): milestones 0–6 feature work is substantially complete. The canonical shared contract, lossless Android import/export adapter, validation/round-trip tests, version-2 web backup envelope, IndexedDB migration, durable Android and web mutation outboxes, enabled entity-sync clients, deployed entity-sync schema, automatic Android retry worker, shared deterministic planning engine, full reference catalog, signed remote catalog updates, shared Hebrew reference/numeral formatter, and shared structured material-range catalog are implemented. Android and web use the same bilingual preset and custom units, verified positions/dates, exact ranges, synchronized exclusions, cadence, pace, finish-by distribution, missed-work behavior, finite/annual chazarah, weekend, and official Oraysa rules. Schedule management, Today, Calendar, progress/goals, synchronized settings, canonical backup/restore, printable schedules, installable offline behavior, reminders while open, due-count badges, and system light/dark mode are implemented. Milestone 7 accessibility validation, browser coverage, sync/migration integration testing, security review, performance budgets, and rollback/recovery testing remain.

Update (2026-09-18): best-effort browser reminders now request permission only from an explicit settings save, schedule the synchronized local reminder time while the web app remains open, use the service worker for localized due-count notifications, and return notification clicks to Today. Reliable closed-app delivery remains a documented Web Push dependency. Web signed preset updates now verify the Android-compatible P-256 envelope with WebCrypto, validate every bundled preset/reference in Kotlin, retain only verified cache entries, and fall back to the bundled catalog. Web also follows the system light/dark preference with Android-matched semantic colors and browser chrome. Accessibility hardening is the next visible parity gap.

Update (2026-09-23): deployment now requires the shared JVM/Wasm, Android unit, and web browser test suites to pass. Web navigation, task completion, collapsible sections, and calendar days expose explicit roles and selected/expanded/completed state, and the shell honors reduced-motion preferences. Formal multi-browser screen-reader, keyboard focus-order, 200% zoom, contrast, and RTL validation remains.
...
- Sync policy tests cover retry classification, duplicate acknowledgements, concurrent edit/delete conflicts, mutation rebasing, account-switch rejection, and token-refresh boundaries. The production web session manager is tested for rotating-token persistence, concurrent refresh coalescing, malformed responses, and retry after failure. A deterministic two-device gate runs independent IndexedDB stores through the deployed server contract for seeding, offline edits, remote-wins conflicts, tombstones, interrupted acknowledgements, idempotent replay, and late-ack replacement rebasing. Live authenticated Android/web verification completes this gate.
- Migration tests cover every supported Android database/backup format and the current web IndexedDB v2/local-storage bootstrap path. Web recovery tests also cover blocked/open failures, interrupted migration, failed or closed writes, pending-fallback restart promotion, and remote-apply failure; destructive real-browser validation remains in the browser matrix.
...
- Service-worker upgrades have atomic install/activation rollback tests; IndexedDB upgrades still need browser-level recovery coverage.
