...
| Cross-device sync | Incremental client enabled | Incremental client enabled | Both use the current entity API and protect account binding and pending local mutations; CI covers two independent browser stores converging through offline edits, tombstones, remote-wins conflicts, interrupted acknowledgements, idempotent replay, and late-ack replacement rebasing, plus retry and account-switch policy. Live authenticated Android/web and end-to-end token-refresh verification remains in milestone 7 |
